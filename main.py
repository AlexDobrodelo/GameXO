import os       # Для использования возможности очистки экрана

def input_name():       # Ввод и сохранение имен игроков
    gamers = [1, 2]
...
def input_size_square():    # Ввод размера игрового поля и
    os.system('cls')        # Очистка экрана перед печатью
    while True:
        #
        result = int(input('''Введите размер игрового поля
3 для поля 3х3, 4 для поля 4х4, 5 для поля 5х5, 6 для поля 6х6.
Игровое поле неможет быть меньше 3 и больше 6 :>'''))
...
def init_square(size_square):  # Функция генерации пустого игрового поля с номерами строк и столбцов
        result = [['-' for i in range(size_square+1)] for j in range(size_square+1) ] # генерируем пустое игровое поле
...
def input_step(gamer): # ввод и проверка координат хода
    ...
    while True:     # Бесконечный цикл пока не ввод не будет коректный
        x, y = map(int, input('Введите номер строки и через пробел номер столбца :>').split(" "))
        if not (0 < x <= size_square  and 0 < y <= size_square):  # Проверка введеных координат
            print(f'{gamer}, вы ввели не верные координаты,'    # на соответствие максимальных и минимальных диапазонов,
                  f'значения столбца и строки должны быть в '   # координаты не должны выходить на пределы игрового поля
                  f'диаразоне от 1 до {size_square}.')
...
gamers = input_name()     # Список с и менами игроков. По умолчанию имена игроков  "1" и "2"

size_square = input_size_square()

game_square = init_square(size_square) # Генерируем игровое поле
